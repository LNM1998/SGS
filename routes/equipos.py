...
def equipos_aios():
    ...
    if filtro_edificio:
        ...
        if filtro_edificio == "Lima":
            if filtro_piso:
                # Si el piso también se especificó, filtramos normalmente (incluso "taller lima")
                consulta = consulta.filter(Equipo.piso == filtro_piso)
            else:
                # Si no hay filtro de piso, excluimos "taller lima"
                consulta = consulta.filter(Equipo.piso != "Taller Lima")
        if filtro_edificio == "Peru":
            if filtro_piso:
                # Si el piso también se especificó, filtramos normalmente (incluso "taller peru")
                consulta = consulta.filter(Equipo.piso == filtro_piso)
            else:
                # Si no hay filtro de piso, excluimos "taller peru"
                consulta = consulta.filter(Equipo.piso != "Taller Peru")
        if filtro_edificio == "ISER":
            if filtro_piso:
                # Si el piso también se especificó, filtramos normalmente (incluso "taller iser")
                consulta = consulta.filter(Equipo.piso == filtro_piso)
            else:
                # Si no hay filtro de piso, excluimos "taller peru"
                consulta = consulta.filter(Equipo.piso != "Taller Iser")  
        elif filtro_piso:
            consulta = consulta.filter(Equipo.piso == filtro_piso)
    ...
    if filtro and valor:
        if filtro == 'maquina_actual':
            ...
            consulta = consulta.filter(Equipo.version_windows.contains(valor))
        elif filtro == 'usuario':
    ...
    if filtro2 and valor2:
        if filtro2 == 'maquina_actual':
            ...
            consulta = consulta.filter(Equipo.version_windows.contains(valor2))
    ...
    return render_template('equipos_aios.html', equipos=equipos, filtro=filtro, valor=valor, filtro2=filtro2, valor2=valor2, filtro_edificio=filtro_edificio, filtro_piso=filtro_piso, filtro_edificio2=filtro_edificio2, filtro_piso2=filtro_piso2)
